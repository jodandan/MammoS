...
const AttachmentLine = styled.div`
  border: 1px solid rgba(0, 0, 0, 0.2);
  box-shadow: 2px 3px 5px 0px rgba(160, 160, 160, 0.5);
`;
    ...
    <PageFrame>
      <PageContainer>
        <PostContainer>
          ...
          <Bottom>
            <Title type="text" defaultValue={post.postTitle}></Title>
            <TitleLine />
            {images.length !== 0 && images.length < 3 && (
              <ImgBox className="center">{makeImages(images)}</ImgBox>
            )}
            {images.length >= 3 && <ImgBox>{makeImages(images)}</ImgBox>}
            <Content defaultValue={post.postContents}></Content>
            <AttachmentBox>
              <div style={{ width: '75%' }}>
                <Attachment></Attachment>
                <AttachmentLine />
              </div>
              <Button className="attachment">완료</Button>
            </AttachmentBox>
            <ButtonBox>
              {isLeader && <Button>완료</Button>}
